...
func TestReadTimeout(t *testing.T) {
	...
	// Send data
	log.Printf("T1 - Starting ECHO test")
	n, err = target.Write([]byte("E")) // 'E' starts echo test
	require.NoError(t, err, "Error sending echo test command")
	require.Equal(t, 1, n, "Write sent a wrong number of bytes")

	log.Printf("T1 - Sending 'X' to target")
	n, err = target.Write([]byte("X")) // 'X' should be repeated
	require.NoError(t, err, "Error sending data to be echoed")
	require.Equal(t, 1, n, "Write sent a wrong number of bytes")
